...
export async function getStaticProps() {
  try {
    const res = await fetch(
      `${baseUrl}/frontend/data/page-data-list-by-group/explore`
    );
    ...
    Object.keys(result.data).forEach((group) => {
      mappedData[group] = result.data[group].map((item) => ({
        id: item.id,
        title: item.title,
        description: item.description,
        images: item.images?.map((img) => img.image_path) || [
          item.image_path || '/assets/explore/placeholder.jpg',
        ],
    ...
    return {
      props: {
        exploreData: mappedData,
      },
      revalidate: 60, // Incremental Static Regeneration every 60 seconds
    };
  } catch (error) {
    console.error('Error fetching explore data in getStaticProps:', error);
    return { props: { exploreData: {} } };
  }
}
...
export default function ExperienceSection({
  ...
  useEffect(() => {
    if (!initialData || Object.keys(initialData).length === 0) {
      ...
      fetch(`${baseUrl}/frontend/data/page-data-list-by-group/explore`)
        .then((res) => {
          if (!res.ok) {
            throw new Error(
      ...
      <div className='container mx-auto px-6 md:px-12 lg:px-20'>
        ...
        {isDataLoading ? (
          <div className='text-center text-gray-600'>
            Loading explore options...
          </div>
        ) : Object.keys(exploreData).length === 0 ? (
          <div className='text-center text-gray-600'>
            No explore options available.
          </div>
        ) : (
