...
export default async (req, res) => {
  ...
  const { name, email, phone, sheetName, checkin, checkout, rooms } = req.body;
  ...
  if (
    ...
    return res.status(400).json({
      message:
        'Missing required fields: name, email, phone, checkin, checkout, rooms, and sheetName are required',
    });
  }
  ...
  try {
    ...
    if (!checkSheet.data.values) {
      await sheets.spreadsheets.values.update({
        ...
        resource: {
          values: [['Name', 'Email', 'Mobile', 'Checkin', 'Checkout', 'Rooms']],
        },
      });
    }
    ...
    return res.status(200).json({
      ...
      data: { name, email, phone, checkin, checkout, rooms },
    });
  } catch (error) {
